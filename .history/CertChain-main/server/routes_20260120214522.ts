...
export async function registerRoutes(
  ...
  app.get(api.verifier.search.path, async (req, res) => {
    ...
    // SAFE FIX:
    const rollNumber = typeof req.query.rollNumber === 'string' ? req.query.rollNumber : null;

    if (!rollNumber) {
      return res.status(400).json({ message: "Roll number required and must be a string" });
    }
