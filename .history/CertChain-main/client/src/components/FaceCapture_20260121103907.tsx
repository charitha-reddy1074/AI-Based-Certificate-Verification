...
export function FaceCapture({ onCapture, label = "Capture Face" }: FaceCaptureProps) {
  ...
  const handleCapture = async () => {
    // 1. Guard against missing refs
    if (!videoRef.current || !modelsLoaded) {
      setError("Camera not ready. Please wait...");
      return;
    }

    // 2. Ensure TensorFlow is ready before every detection
    try {
      await tf.ready();
    } catch (e) {
      console.error("TF not ready:", e);
      setError("TensorFlow initialization failed. Please refresh the page.");
      setIsCapturing(false);
      return;
    }
    ...
    try {
      ...
      if (modelsAvailable) {
        try {
          // 3. Check if video is actually playing and has data
          ...
          const detections = await faceapi
            .detectSingleFace(videoRef.current) // This is where the 'backend' error triggers
            .withFaceLandmarks()
            .withFaceDescriptor();
          ...
          console.warn("Detection error caught:", detectionError.message);
          // If it's the 'backend' error, we fall through to the mock logic
        }
      } else {
