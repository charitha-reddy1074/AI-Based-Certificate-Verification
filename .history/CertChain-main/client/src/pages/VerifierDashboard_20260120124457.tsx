...
export default function VerifierDashboard() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <header className="bg-gradient-to-r from-primary to-secondary text-background sticky top-0 z-40">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-8 space-y-12">
        ...
        <section className="max-w-2xl mx-auto text-center space-y-6">
          <h2 className="text-3xl font-display font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-secondary">Verify Academic Credentials</h2>
          <form onSubmit={handleSearch} className="flex gap-4">
            ...
            <Button size="lg" className="h-12 px-8 bg-gradient-to-r from-primary to-secondary text-background hover:from-primary/90 hover:to-secondary/90" disabled={search.isPending}>
          ...
          <div className="grid gap-4 text-left">
            ...
            {searchResults.map((cert) => (
              <div key={cert.id} className="bg-card border-border border p-4 rounded-lg shadow flex justify-between items-center hover:border-primary/50 transition-colors">
                ...
                <Button onClick={() => setSelectedCertId(cert.id)} className="bg-gradient-to-r from-primary to-secondary text-background hover:from-primary/90 hover:to-secondary/90">
                  View Certificate
                </Button>
              </div>
            ))}
      ...
      <Dialog open={!!selectedCertId} onOpenChange={() => setSelectedCertId(null)}>
        <DialogContent className="bg-card border-border">
          ...
          <div className="bg-gradient-to-br from-primary/10 to-secondary/10 border border-primary/20 p-6 rounded-lg text-center my-4">
            <p className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-secondary">₹10.00</p>
